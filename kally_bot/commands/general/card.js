...
module.exports = {
	name: "card",
	description: "Display information for a specified card.",
	usage: `${prefix}card <card_name>`,
  developer: false, 
	...
	execute(client, message, args, Discord) {
		...
		let db = Object.keys(client.db.cards);
		...
		if(db.includes(query) || db.includes( query.split("'").join('') )){
			display_card(message, Discord, eval(`client.db.cards.${ query.split("'").join('') }`));
		}else{
			let current_step = db;
			let previous_step = [];
			...
			if(current_step[0] != undefined){
				display_card(message, Discord, eval(`client.db.cards.${ current_step[0].split("'").join('') }`));
			}else{
...
function display_card(message, Discord, card){
	...
	const exampleEmbed = new Discord.MessageEmbed()
		...
		message.reply({embeds: [exampleEmbed], files: [attachment]}).catch(console.error)
}