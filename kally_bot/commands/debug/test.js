...
let { MessageActionRow, MessageButton, MessageEmbed } = require('discord.js');
			...
			return new MessageEmbed({
				...
				fields: await Promise.all(
					current.map(async guild => ({
						name: guild[1].name,
						value: `**Name:** ${guild[1].name}\n**Description:** ${guild[1].description}`//(await guild.fetchOwner()).user.tag
					}))
				)
			});
		}
